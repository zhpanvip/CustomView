...
package com.cypoem.wheel_view.wheel;
...
import com.cypoem.wheel_view.R;
...
//			0x00AAAAAA, 0x00AAAAAA };
	...
	private static int ADDITIONAL_ITEM_HEIGHT =0;
...
//	private GradientDrawable bottomShadow;
	...
	private void initData(Context context) {
		...
		ADDITIONAL_ITEM_HEIGHT=30* (int)dm.density;
	...
	private void initResourcesIfNecessary() {
		TEXT_SIZE=(int)this.getResources().getDimension(R.dimen.contenttitlesize);
		if (itemsPaint == null) {
	...
	private int calculateLayoutWidth(int widthSize, int mode) {
		...
		if (maxLength > 0) {
			/*float textWidth = FloatMath.ceil(Layout.getDesiredWidth("0",
					itemsPaint));*/
			float textWidth=itemsPaint.measureText("0");
		...
		if (label != null && label.length() > 0) {
			/*labelWidth = (int) FloatMath.ceil(Layout.getDesiredWidth(label,
					valuePaint));*/
			labelWidth= (int) valuePaint.measureText(label);
		}
